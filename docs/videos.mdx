...
`POST /v1/videos` submits an asynchronous video generation job. Credits are held when the job starts and committed when it completes.
...
| `image_url` | For image-to-video | Source image URL |
| `video_url` | For video-to-video | Source video URL |
